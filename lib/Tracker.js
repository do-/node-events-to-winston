...
const LOGGING_DETAILS= Symbol ('details')
...
class Tracker {
	...
	static LOGGING_DETAILS = LOGGING_DETAILS
	...
	constructor (emitter, logger, options = {}) {
		...
		for (const [event, o] of Object.entries (options.events)) {
			...
			if (o.details != null && typeof o.details !== 'object' && typeof o.details !== 'function') throw Error (`Invalid details for ${event}: ${o.details}`)
	...
	getInfo (event, payload) {
		...
		if ('details' in info && LOGGING_DETAILS in emitter) info.details = {

			...emitter [LOGGING_DETAILS],
		...
		if (conf.elapsed) {
			...
			info.elapsed = Date.now () - this.start

			if (!('isLast' in conf)) {

				for (const e of Object.values (this.events)) if (e.isLast) {
			...
			if (!('isLast' in conf)) conf.isLast = true

		}

		if (conf.isLast) info.isLast = true
