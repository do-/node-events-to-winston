...
const restrictReplacer = (f, maxLength, ellipsis, enc) => {
	...
	const encode = getEncoder (enc), cut = v => {
		...
		if (typeof v === 'object' && v !== null && v.type === 'Buffer' && Array.isArray (v.data) && Object.keys (v).length === 2) v = encode (Buffer.from (v))
